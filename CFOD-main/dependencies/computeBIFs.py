...
from dependencies.DtGfiltersBank import DtGfiltersBank
from dependencies.efficientConvolution import efficientConvolution
def compute_bifs(im, sigma, epsilon, configuration=1):
    ...
    for i, order in enumerate(orders):
        jet[i, :, :] = efficientConvolution(im, DtGfilters[i,0,:],DtGfilters[i,1,:]) * (sigma ** (sum(order)))
    jet = np.array(jet)
