...
gabor_intra={};
fileList={};
intra_file_list={}; % save the sequence of processing the CT file. So we can know what each row corresponding each patients
% laplace_intra ={};
...
for z = 3:length(Files)
    ...
    fprintf(2,'\n%s\n',FileNames);
    cd(FileNames);
    intra_file_list{end+1} = FileNames; % Save the file list
...
%          HU_intra{z,1}=traina_HU;
         law_intra{(z-2),1}=traina_law;
%          laplace_intra{(z),1}=traina_laplace;
         haralick_intra{(z-2),1}=traina_haralick;
         gabor_intra{(z-2),1}=traina_gabor;
         
        cd('E:\GT\Research\CCF\ccf_feature_results_separate\Adenocarcinoma\intra_feature')
