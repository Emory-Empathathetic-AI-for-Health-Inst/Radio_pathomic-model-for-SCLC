%Prateek 7/1/2015
function [feat]=compute_CoLlAGe2D(origImage, tumorMask, k)
%close all
% Med_entropy=[];
% Mean_entropy=[];
% index_array=[];
...
% x2 =  round(max(pos(:,2)));
% y2 =  round(max(pos(:,1)));
% I2 = I.*double(BW); % apply mask to image
...
%            if BW1(i,j)==1
%            %arrowlength=2;
%            h= quiver(j, i, cos(dominant_orientation_roi(i,j)), sin(-dominant_orientation_roi(i,j)),'b','linewidth',2,'MaxHeadSize',1,'AutoScaleFactor',0.5,'AutoScale','off');
%            %pause
