...
law_peri = cell(numFiles, 1);
haralick_peri = cell(numFiles, 1);
gabor_peri = cell(numFiles, 1);
peri_file_list = cell(numFiles, 1);
...
parfor z=3:length(Files)   
    ...
    fprintf(2,'\n%s\n',FileNames);
    cd(FileNames);
    V1a = niftiread('CT_T1.nii.gz');
    V2a = niftiread('CT_T1-label.nii.gz');
...
%                     laplace_features=0;
         ...
         % Store the result
         law_peri{z-2} = traina_law;
         haralick_peri{z-2} = traina_haralick;
         gabor_peri{z-2} = traina_gabor;
         peri_file_list{z-2} = FileNames;
         
         
end
cd('E:\GT\Research\NLST\Cohort1_T1_Cohort2_T2_resample_thickness\Cohort1_intra_peri\peri_feature')
