...
result_path = r'E:\GT\Research\NLST\small_test\TEX\peri'
...
def generate_feature_names():
    ...
    # Gabor feature names
    gabor_features = [f'Peri_Gabor_f{f}_theta{t}_{s}'
    ...
    laws_features = [f'Peri_Laws_{a}{b}_{s}'
                     for a in laws_bases
                     for b in laws_bases
                     for s in stats]

    # Haralick feature names
    haralick_features = [f'Peri_Haralick_{i}_{s}'
                         for i in range(1, 14)
                         for s in stats]
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(peri_features, columns=feature_names)

    # Add nodule ID column
    df.insert(0, 'nodule_id', nodule_ids)

    # Save to CSV
    output_csv_path = os.path.join(result_path, 'peri_features.csv')
